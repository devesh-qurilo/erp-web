// // components/MilestonesTable.tsx
// "use client";
...
//           throw new Error(`HTTP ${res.status} ${res.statusText} — server returned HTML. Snippet: ${text.slice(0, 300)}`);
//         }
...
//   // open create modal
//   const openCreate = () => {
//     setForm({ title: "", milestoneCost: "", taskCount: "0", status: "INCOMPLETE" });
//     setSaveError(null);
...
//         taskCount: Number(form.taskCount || 0),
...
//         return;
//       }
//       const payload = { status: String(newStatus).toUpperCase() };
//       const url = buildUrl(`${gatewayPath}/${parsedProjectId}/milestones/${m.id}/status`);
//       // use PUT (server-side may accept PATCH / PUT — your server can adjust)
//       await doFetch(url, {
//         method: "PATCH",
//         headers: headers({ "Content-Type": "application/json" }),
//         credentials: "same-origin",
...
//     return st === "complete" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed";
//   };

//   // simpler color helpers (also tolerant)
//   const statusClasses = (s?: string) => {
//     const st = String(s || "").toLowerCase();
//     if (st === "complete" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed" || st === "completed") {
//       return "bg-green-50 text-green-700 border-green-200";
//     }
//     if (st === "completed" || st === "completed") {
//       return "bg-green-50 text-green-700 border-green-200";
//     }
//     // Accept uppercase tokens too
//     if (String(s) === "COMPLETED") return "bg-green-50 text-green-700 border-green-200";
...
//                 {/* STATUS with dropdown - display uppercase tokens */}
//                 <div className="col-span-2 text-sm">
//                   <div className="relative inline-block">
//                     <button
//                       onClick={() => setStatusMenuOpenFor(statusMenuOpenFor === m.id ? null : (m.id ?? null))}
//                       className={`inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs ${statusClasses(m.status)}`}
//                       title="Change status"
//                     >
//                       <span className={`w-2 h-2 rounded-full ${dotColor(m.status)}`} />
//                       <span>{String(m.status || "INCOMPLETE").toUpperCase()}</span>
//                     </button>
...
//                       <button
//                         onClick={() => { openView(m); setActionOpenFor(null); }}
...
//                   <input type="text" value={form.title} onChange={(e) => setForm((s) => ({ ...s, title: e.target.value }))} className="w-full border rounded px-3 py-2" placeholder="Milestone Title" />
//                 </div>

//                 <div>
//                   <label className="block text-sm text-gray-700 mb-2">Milestone Cost</label>
//                   <input type="number" value={form.milestoneCost} onChange={(e) => setForm((s) => ({ ...s, milestoneCost: e.target.value }))} className="w-full border rounded px-3 py-2" placeholder="0.00" min={0} />
//                 </div>

//                 <div>
//                   <label className="block text-sm text-gray-700 mb-2">Task Count</label>
//                   <input type="number" value={form.taskCount} onChange={(e) => setForm((s) => ({ ...s, taskCount: e.target.value }))} className="w-full border rounded px-3 py-2" min={0} />
//                 </div>

//                 <div>
//                   <label className="block text-sm text-gray-700 mb-2">Status</label>
//                   <select value={form.status} onChange={(e) => setForm((s) => ({ ...s, status: e.target.value }))} className="w-full border rounded px-3 py-2">
...
//                 <button onClick={() => setIsCreateOpen(false)} className="px-6 py-2 rounded-md border text-blue-600" disabled={saving}>Cancel</button>
//                 <button onClick={createMilestone} className="px-6 py-2 rounded-md bg-blue-600 text-white" disabled={saving}>{saving ? "Saving..." : "Save"}</button>
//               </div>
//             </div>
//           </div>
//         </div>
...
//                 <div><strong>Milestone Cost:</strong> {formatCurrency(selected.milestoneCost)}</div>
//                 <div><strong>Task Count:</strong> {selected.taskCount ?? 0}</div>
//                 <div><strong>Status:</strong> {String(selected.status ?? "INCOMPLETE").toUpperCase()}</div>
//                 <div><strong>Created At:</strong> {selected.createdAt ? new Date(selected.createdAt).toLocaleString() : "-"}</div>
//                 <div><strong>Raw:</strong> <pre className="text-xs bg-gray-50 p-2 rounded">{JSON.stringify(selected, null, 2)}</pre></div>
//               </div>
...
//                 <div>
//                   <label className="block text-sm text-gray-700 mb-2">Title *</label>
//                   <input type="text" value={form.title} onChange={(e) => setForm((s) => ({ ...s, title: e.target.value }))} className="w-full border rounded px-3 py-2" />
//                 </div>

//                 <div>
//                   <label className="block text-sm text-gray-700 mb-2">Milestone Cost</label>
//                   <input type="number" value={form.milestoneCost} onChange={(e) => setForm((s) => ({ ...s, milestoneCost: e.target.value }))} className="w-full border rounded px-3 py-2" min={0} />
//                 </div>
...
//               <div className="flex items-center justify-center gap-6 mt-6 pb-6">
//                 <button onClick={() => setIsEditOpen(false)} className="px-6 py-2 rounded-md border text-blue-600" disabled={saving}>Cancel</button>
//                 <button onClick={updateMilestone} className="px-6 py-2 rounded-md bg-blue-600 text-white" disabled={saving}>{saving ? "Saving..." : "Save"}</button>
//               </div>
//             </div>
//           </div>
//         </div>
...
//               <p className="text-sm text-gray-700">Are you sure you want to delete milestone <strong>{selected.title}</strong> ? This action cannot be undone.</p>

//               {saveError && <div className="mt-4 text-sm text-red-600">{saveError}</div>}

//               <div className="flex items-center justify-end gap-4 mt-6">
//                 <button className="px-4 py-2 rounded border" onClick={() => setIsDeleteConfirmOpen(false)} disabled={saving}>Cancel</button>
//                 <button className="px-4 py-2 rounded bg-red-600 text-white" onClick={deleteMilestone} disabled={saving}>{saving ? "Deleting..." : "Delete"}</button>
//               </div>
//             </div>
//           </div>
//         </div>
...
export default function MilestonesTable({
  ...
  const changeStatus = async (m: Milestone, newStatus: "COMPLETED" | "INCOMPLETE" | string) => {
    ...
    try {
      ...
      await doFetch(url, {
        method: "PATCH", // keep PATCH (server may accept PATCH/PUT - adjust if needed)
